...
<div class="bg-white p-6 rounded shadow-md m-4">
    ...
    <div class="flex items-center justify-between font-text my-4">
        ...
        <a href="{{ route('products.index') }}" aria-label="Gå tillbaka till produkter" class="bg-slate-600 hover:bg-slate-800 text-white font-bold py-2 px-4 rounded cursor-pointer">Tillbaka till produkter</a>
    </div>
    ...
    <form action="{{ route('products.update', ['product' => $product->id]) }}" method="POST" enctype="multipart/form-data" aria-label="Redigera produkt-formulär" class="flex flex-col gap-4 font-text">
        ...
        <button type="submit" aria-label="Uppdatera produkt" class="bg-slate-500 hover:bg-slate-700 text-white font-bold py-2 px-4 rounded">
            Uppdatera produkt
        </button>
