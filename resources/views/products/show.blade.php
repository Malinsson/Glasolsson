...
<section aria-label="Produktinformation">
    ...
    <section class="font-text flex flex-col gap-3 md:flex-row md:gap-6 pb-6">
        <div class="font-text pb-6 pt-6 w-full">
            <h2 class="text-2xl"><strong>{{$product->name}}</strong></h2> 
            {{ Breadcrumbs::render('products.show', $product) }}
            <a href="{{ route('products.index') }}"
            aria-label="Gå tillbaka till produkter"
            class="inline-flex items-center mt-6 bg-slate-600 hover:bg-slate-800 text-white py-2 px-4 rounded cursor-pointer focus:outline-none focus:ring-2 focus:ring-blue-500">
