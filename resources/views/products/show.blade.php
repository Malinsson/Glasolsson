...
<div class="flex flex-row justify-between gap-3 m-8 md:justify-start">
    <a href="{{ route('products.edit', $product->id) }}" class="bg-slate-600 hover:bg-slate-800 text-white py-2 px-4 rounded cursor-pointer"><strong>Redigera produkt</strong></a>

    <form id="delete-form" action="{{ route('products.destroy', ['product' => $product->id]) }}" method="POST" style="display:inline;">
        @method('DELETE')
        @csrf
        <button id="open-delete-modal" type="button" class="bg-red-600 hover:bg-red-800 text-white py-2 px-4 rounded cursor-pointer"><strong>Radera produkt</strong></button>
    </form>
